...
void inputaika_l2(INT32 *t,INT32 t0, int l, int x, int y, char *term, char *tc)
   ...
   do {
      ...
      if ((tt = strtoaika_l2(tstr,t0)) != -24*36000L) ok = 1;
      else {
         erbeep();
         tstr0[0] = 0;
         for (i = 0; i < 10; i++) {
...
void inputaika_l2w(INT32 *t, int t0, int l, int x, int y, wchar_t *wterm, wchar_t *wtc)
{
	char term[40] = "", tc;
	int i;
	...
	for (i = 0; wterm[i]; i++)
		if (wterm[i] >= 201 && wterm[i] <= 210)
			term[i] = (char) wterm[i];
	inputaika_l2(t, t0, l, x, y, term, &tc);
	if (tc >= 201 && tc <= 210)
		*wtc = tc;
	else
		*wtc = oemtowchar(tc);
