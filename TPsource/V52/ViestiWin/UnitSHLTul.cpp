		...
		if (TulosteForm->SarjaValittu(srj)) {
			...
			for (int os = 0; os < Sarjat[srj].ntosuus; os++) {
				if (os == 0) {
					...
					SG1->Cells[0][nsrj+1] = L"";
					SG1->Cells[1][nsrj+1] = L"";
					}
				SG1->Cells[2][nsrj+1] = os+1;
				ansitowcs(st, Sarjat[srj].matka[os], 10);
				if (wcslen(st) > 1) {
					for (p = st+wcslen(st)-1; p > st && wcswcind(*p, L"pPvV") >= 0; p--);
					if (++p < st+wcslen(st)) {
						SG1->Cells[4][nsrj+1] = UnicodeString(p).UpperCase();
						}
	...
	for (int srj = 0; srj < sarjaluku + nsarjayhd; srj++) {
		if (TulosteForm->SarjaValittu(srj)) {
			for (int os = 0; os < Sarjat[srj].ntosuus; os++) {
				...
				wcsncpy(TulosteForm->SHLMatka[srj][os], SG1->Cells[3][nsrj+1].c_str(), sizeof(TulosteForm->SHLMatka[0])/2-1);
				wcsncpy(TulosteForm->SHLTyyli[srj][os], SG1->Cells[4][nsrj+1].c_str(), sizeof(TulosteForm->SHLTyyli[0])/2-1);
				nsrj++;
				}
			}
		}
...
void __fastcall TFormSHLtul::RGtyylitClick(TObject *Sender)
{
	...
	for	(int r = 1; r < SG1->RowCount; r++)
		SG1->Cells[4][r] = L"PV"[RGtyylit->ItemIndex-1];
}
//---------------------------------------------------------------------------
