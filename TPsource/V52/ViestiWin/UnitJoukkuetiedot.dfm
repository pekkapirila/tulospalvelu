object FormJoukkuetiedot: TFormJoukkuetiedot
  Left = 613
  Top = 228
  Caption = 'Joukkuetiedot'
  ClientHeight = 634
  ClientWidth = 803
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Arial'
  Font.Style = []
  Icon.Data = {
    }
  Menu = MainMenu1
  OldCreateOrder = False
  Scaled = False
  OnClose = FormClose
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnResize = FormResize
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 14
  object Label1: TLabel
    Left = 6
    Top = 21
    Width = 30
    Height = 14
    Caption = 'KilpNo'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
  end
  object Label10: TLabel
    Left = 8
    Top = 62
    Width = 25
    Height = 14
    Caption = 'Sarja'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
  end
  object Tila: TLabel
    Left = 6
    Top = 5
    Width = 121
    Height = 16
    Caption = 'Haku- ja katselutila'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = [fsBold]
    ParentFont = False
  end
  object Label2: TLabel
    Left = 58
    Top = 21
    Width = 29
    Height = 14
    Caption = 'Seura'
  end
  object LblSeuralyh: TLabel
    Left = 414
    Top = 23
    Width = 57
    Height = 14
    Caption = 'Joukkuenimi'
  end
  object LblMaa: TLabel
    Left = 362
    Top = 23
    Width = 20
    Height = 14
    Caption = 'Maa'
  end
  object Label6: TLabel
    Left = 378
    Top = 62
    Width = 52
    Height = 14
    Caption = 'Seurahaku'
  end
  object LblJoukkue: TLabel
    Left = 313
    Top = 21
    Width = 39
    Height = 14
    Caption = 'Joukkue'
  end
  object Label7: TLabel
    Left = 531
    Top = 21
    Width = 48
    Height = 14
    Caption = 'Id-numero'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
  end
  object Label4: TLabel
    Left = 147
    Top = 62
    Width = 32
    Height = 14
    Caption = 'Pisteet'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
  end
  object Label11: TLabel
    Left = 604
    Top = 21
    Width = 39
    Height = 14
    Caption = 'Piiri/alue'
  end
  object Label3: TLabel
    Left = 376
    Top = 101
    Width = 48
    Height = 28
    Caption = 'Selattava osuus'
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
    WordWrap = True
  end
  object EdtKilpno: TEdit
    Left = 8
    Top = 35
    Width = 44
    Height = 24
    Alignment = taRightJustify
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    Text = '0'
    OnExit = EdtKilpnoChanged
    OnKeyDown = EdtKeyDown
    OnKeyPress = EdtKilpnoKeyPress
  end
  object EdtSeura: TEdit
    Left = 58
    Top = 35
    Width = 246
    Height = 24
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
    ReadOnly = True
    TabOrder = 1
    OnExit = EdtSeuraExit
    OnKeyDown = EdtKeyDown
  end
  object EdtMaa: TEdit
    Left = 362
    Top = 35
    Width = 45
    Height = 24
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
    ReadOnly = True
    TabOrder = 3
    OnExit = EdtMaaExit
    OnKeyDown = EdtKeyDown
  end
  object SrjKarki: TButton
    Left = 501
    Top = 106
    Width = 66
    Height = 20
    Caption = 'Sarjan &k'#228'rki'
    TabOrder = 14
    OnClick = SrjKarkiBtn
  end
  object SeurBtn: TButton
    Left = 633
    Top = 105
    Width = 54
    Height = 20
    Caption = '&Seuraava'
    TabOrder = 16
    OnClick = SeurBtnClick
  end
  object EdBtn: TButton
    Left = 573
    Top = 105
    Width = 54
    Height = 20
    Caption = '&Edellinen'
    TabOrder = 15
    OnClick = EdBtnClick
  end
  object SrjVal: TComboBox
    Left = 8
    Top = 76
    Width = 123
    Height = 24
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
    TabOrder = 4
    OnChange = SrjValChange
    OnExit = SrjValExit
    OnKeyDown = EdtKeyDown
  end
  object BtnSalli: TButton
    Left = 8
    Top = 118
    Width = 75
    Height = 41
    Caption = 'Salli muokkaus'
    ImageAlignment = iaCenter
    TabOrder = 18
    WordWrap = True
    OnClick = BtnSalliClick
  end
  object BtnTallenna: TButton
    Left = 189
    Top = 118
    Width = 75
    Height = 41
    Caption = 'Tallenna muutokset'
    ImageAlignment = iaCenter
    TabOrder = 6
    Visible = False
    WordWrap = True
    OnClick = BtnTallennaClick
  end
  object BtnPeruuta: TButton
    Left = 281
    Top = 118
    Width = 75
    Height = 41
    Caption = 'Peruuta muutokset'
    ImageAlignment = iaCenter
    TabOrder = 7
    Visible = False
    WordWrap = True
    OnClick = BtnPeruutaClick
  end
  object BtnSulje: TButton
    Left = 705
    Top = 118
    Width = 75
    Height = 41
    Caption = 'Sulje ikkuna'
    ImageAlignment = iaCenter
    TabOrder = 19
    WordWrap = True
    OnClick = BtnSuljeClick
  end
  object OsGrid: TStringGrid
    Left = 4
    Top = 173
    Width = 791
    Height = 172
    DefaultRowHeight = 20
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Arial'
    Font.Style = []
    Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goRangeSelect, goColSizing, goTabs]
    ParentFont = False
    TabOrder = 5
    OnClick = OsGridClick
    OnDblClick = OsGridDblClick
    OnExit = OsGridExit
    OnKeyDown = OsGridKeyDown
    OnKeyPress = OsGridKeyPress
    OnSelectCell = OsGridSelectCell
  end
  object EdtJoukkue: TEdit
    Left = 311
    Top = 35
    Width = 45
    Height = 24
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
    ReadOnly = True
    TabOrder = 2
    OnExit = EdtJoukkueExit
    OnKeyDown = EdtKeyDown
  end
  object EdtJoukkNimi: TEdit
    Left = 413
    Top = 35
    Width = 110
    Height = 24
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
    ReadOnly = True
    TabOrder = 8
    OnChange = EdtSeuraChange
    OnEnter = EdtJoukkNimiEnter
    OnExit = EdtJoukkNimiExit
  end
  object RGSelaus: TRadioGroup
    Left = 541
    Top = 66
    Width = 173
    Height = 34
    Caption = ' Selausperuste '
    Columns = 3
    ItemIndex = 0
    Items.Strings = (
      'Sijoitus'
      'KilpNo'
      'Seura')
    TabOrder = 20
    TabStop = True
  end
  object EdtNimihaku: TEdit
    Left = 378
    Top = 76
    Width = 146
    Height = 24
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
    TabOrder = 17
    OnChange = EdtNimihakuChange
    OnKeyDown = EdtKeyDown
  end
  object EdtJoukkId: TEdit
    Left = 531
    Top = 35
    Width = 67
    Height = 24
    Alignment = taRightJustify
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
    TabOrder = 9
    Text = '0'
    OnExit = EdtJoukkIdExit
    OnKeyDown = EdtKeyDown
  end
  object EdPisteet0: TEdit
    Left = 147
    Top = 76
    Width = 53
    Height = 24
    Alignment = taRightJustify
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
    TabOrder = 11
    Text = '0'
    OnExit = EdPisteet0Exit
    OnKeyDown = EdtKeyDown
  end
  object EdPisteet1: TEdit
    Left = 207
    Top = 76
    Width = 54
    Height = 24
    Alignment = taRightJustify
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
    ReadOnly = True
    TabOrder = 12
    Text = '0'
    OnExit = EdPisteet1Exit
    OnKeyDown = EdtKeyDown
  end
  object BitBtn1: TBitBtn
    Left = 728
    Top = 8
    Width = 31
    Height = 31
    Margins.Left = 0
    Margins.Right = 0
    Margins.Bottom = 0
    Glyph.Data = {
      
      0000}
    TabOrder = 21
    TabStop = False
    OnClick = BitBtn1Click
  end
  object EdPisteet2: TEdit
    Left = 267
    Top = 76
    Width = 56
    Height = 24
    Alignment = taRightJustify
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
    ReadOnly = True
    TabOrder = 13
    Text = '0'
    OnExit = EdPisteet2Exit
    OnKeyDown = EdtKeyDown
  end
  object EdtPiiri: TEdit
    Left = 604
    Top = 35
    Width = 42
    Height = 24
    Alignment = taRightJustify
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
    ReadOnly = True
    TabOrder = 10
    Text = '0'
    OnExit = EdtPiiriExit
    OnKeyDown = EdtKeyDown
  end
  object CBSelOsuus: TComboBox
    Left = 432
    Top = 103
    Width = 63
    Height = 24
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -13
    Font.Name = 'Arial'
    Font.Style = []
    ParentFont = False
    TabOrder = 22
    OnChange = SrjKarkiBtn
  end
  object TlsGrid: TStringGrid
    Left = 4
    Top = 351
    Width = 791
    Height = 275
    DefaultRowHeight = 20
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Arial'
    Font.Style = []
    Options = [goFixedVertLine, goFixedHorzLine, goVertLine, goHorzLine, goRangeSelect, goColSizing, goTabs]
    ParentFont = False
    TabOrder = 23
    OnClick = TlsGridClick
    OnExit = TlsGridExit
    OnKeyDown = TlsGridKeyDown
    OnKeyPress = TlsGridKeyPress
    OnSelectCell = TlsGridSelectCell
  end
  object RGVaTieto: TRadioGroup
    Left = 427
    Top = 128
    Width = 267
    Height = 37
    Caption = ' N'#228'yt'#228' v'#228'liajoista '
    Columns = 5
    ItemIndex = 1
    Items.Strings = (
      'Kello'
      'Tulos'
      'OsTls'
      'Sija'
      'Matka')
    TabOrder = 24
    OnClick = RGVaTietoClick
  end
  object BtnJjVaihto: TButton
    Left = 98
    Top = 118
    Width = 75
    Height = 41
    Caption = 'Juoksuj'#228'rj.- vaihdot'
    ImageAlignment = iaCenter
    TabOrder = 25
    Visible = False
    WordWrap = True
    OnClick = BtnJjVaihtoClick
  end
  object BtnPaivita: TButton
    Left = 136
    Top = 118
    Width = 75
    Height = 41
    Caption = 'P'#228'ivit'#228
    TabOrder = 26
    OnClick = BtnPaivitaClick
  end
  object MainMenu1: TMainMenu
    Left = 198
    Top = 65533
    object iedosto1: TMenuItem
      Caption = '&Tiedosto'
      object Liskilpailija1: TMenuItem
        Caption = '&Lis'#228#228' kilpailija'
        OnClick = Liskilpailija1Click
      end
      object oistuvienlisystentila1: TMenuItem
        Caption = '&Toistuvien lis'#228'ysten tila'
        OnClick = oistuvienlisystentila1Click
      end
      object Poistakilpailija1: TMenuItem
        Caption = '&Poista kilpailija'
        OnClick = Poistakilpailija1Click
      end
      object N1: TMenuItem
        Caption = '-'
      end
      object Suljekaavake1: TMenuItem
        Caption = '&Sulje kaavake'
        OnClick = Suljekaavake1Click
      end
    end
    object Haku1: TMenuItem
      Caption = '&Haku'
      object Emitkoodihaku1: TMenuItem
        Caption = '&Emit-koodihaku'
        OnClick = Emitkoodihaku1Click
      end
      object Henkilhaku1: TMenuItem
        Caption = '&Henkil'#246'haku'
        OnClick = Henkilhaku1Click
      end
    end
    object Nyt1: TMenuItem
      Caption = '&N'#228'yt'#228
      object Lisenssinumero1: TMenuItem
        Caption = '&Lisenssinumero'
        OnClick = Lisenssinumero1Click
      end
      object Badge1: TMenuItem
        Caption = '&Badge'
        Checked = True
        OnClick = Badge1Click
      end
    end
  end
end
