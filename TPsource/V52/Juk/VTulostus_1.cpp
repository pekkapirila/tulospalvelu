	...
	if (tulprm->tulmuot.cssfl == 1)
		...
		if (tulprm->tulmuot.tauluhtml) {
			...
			if (tulprm->tulostettava == L'E') {
				...
				fsz = tulprm->tulmuot.fontsize + fsz;
				swprintf(st,
					...
					L".ab {background-color: #%6.6x; color: #%6.6x;}\n"
					L"th {background-color: #%6.6x; color: #%6.6x; font-weight: %s; }\n",
					(tulprm->tulmuot.tauluhtml & 2) ? L"width:100%;" : L"",
				...
				tulprm->writehtml(st);
				swprintf(st,
					L".csj {text-align: right; min-width: %dpx; }\n"
					L".cno {text-align: right; min-width: %dpx; }\n"
					L".cjk {text-align: left; min-width: %dpx; }\n"
					L".cmaa {text-align: left; min-width: %dpx; }\n"
					L".cnm {text-align: left; min-width: %dpx; }\n"
					L".ctls {text-align: right; min-width: %dpx; }\n",
					colWh[0]*fsz/fsz0, colWh[0]*fsz/fsz0, colWh[2]*fsz/fsz0,
					colW[0]*fsz/fsz0, colW[3]*fsz/fsz0, colW[5]*fsz/fsz0
					);
				tulprm->writehtml(st);
				}
			swprintf(st,
...
void putfld_class(tulostusparamtp *tulprm, wchar_t *str, wchar_t *cl_str, INT pos, INT len, INT oik)
{
	if (tulprm->kohde != L'H' || tulprm->tulmuot.tauluhtml < 2) {
		putfld(tulprm, str, pos, len, oik, 0);
		return;
		}
	swprintf(tulprm->sline+wcslen(tulprm->sline), L"<td %s%s'>%s</td>",
		cl_str[0] ? L"class='" : (oik ? L"align='right" : L"align='left), cl_str, str);
}
...
void putcell(tulostusparamtp *tulprm, wchar_t *str, INT pos, INT len, INT oik, wchar_t *param, int cellfl)
{
	...
	if (param[0])
		swprintf(tulprm->sline + wcslen(tulprm->sline), L"<td %s>%s</td>", param, str);
	else
		swprintf(tulprm->sline + wcslen(tulprm->sline), L"<td align=%s>%s</td>",
	...
	FldFrmtTp *fld, int (*kntjrj_2)[3], int n_knt)
{
	...
	wchar_t as[26], st[80], stsj[20] = L"", *p, class_str[40] = L"";
#ifdef FIS
	...
	if (!tulprm->tiivis) {
		if (tulprm->viimos == L'K') {
			if (tulprm->tulmuot.tauluhtml) {
				if (tulprm->tiivis)
					wcscpy(class_str, L" class='jktiiv'");
				else
					wcscpy(class_str, L" class='jkk'");
				wcscat(tulprm->sline, L"<tr>");
				}
			else {
	...
	for (int ifld = 0; ifld < n_knt; ifld++) {
		...
		switch (kntjrj_2[ifld][0]) {
			case F_SJ:
				wcscpy(st, stsj);
				wcscat(class_str, L" class='csj'");
				break;
			case F_KNO:
				if (tulprm->tulmuot.knofl || tulprm->tulbadge)
					_itow(kilp->KilpNo(true), st, 10);
				wcscat(class_str, L" class='cno'");
				break;
			case F_JOUK:
				kilp->Joukkue(st, fld[F_JOUK].len);
				wcscat(class_str, L" class='cjk'");
				break;
			case F_JOUKNM:
				kilp->Joukknimi(st, fld[F_JOUKNM].len);
				wcscat(class_str, L" class='cnm'");
				break;
			case F_JOUKID:
