object FormKohdistus: TFormKohdistus
  Left = 0
  Top = 0
  Caption = 'Kartan kohdistus'
  ClientHeight = 336
  ClientWidth = 538
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object GroupBox1: TGroupBox
    Left = 16
    Top = 24
    Width = 292
    Height = 145
    Caption = ' Ensimm'#228'inen kohdistuspiste '
    TabOrder = 0
    object Label1: TLabel
      Left = 20
      Top = 16
      Width = 49
      Height = 13
      Caption = 'Rastikoodi'
    end
    object Label2: TLabel
      Left = 20
      Top = 56
      Width = 96
      Height = 13
      Caption = 'Sijainti kartalla (mm)'
    end
    object Label3: TLabel
      Left = 20
      Top = 96
      Width = 105
      Height = 13
      Caption = 'Sijainti n'#228'yt'#246'll'#228' (pixel)'
    end
    object Label12: TLabel
      Left = 176
      Top = 20
      Width = 97
      Height = 52
      Caption = 
        'N'#228'yt'#228' rastit kartalla ja valitse molemmat rastit ennen muita toi' +
        'mia.'
      WordWrap = True
    end
    object EdtKdi1: TEdit
      Left = 20
      Top = 32
      Width = 33
      Height = 21
      Alignment = taRightJustify
      TabOrder = 0
      Text = '0'
      OnExit = EdtKdi1Exit
      OnKeyPress = EdtKdi1KeyPress
    end
    object EdtMapX1: TEdit
      Left = 20
      Top = 72
      Width = 44
      Height = 21
      Alignment = taRightJustify
      Enabled = False
      ReadOnly = True
      TabOrder = 1
      Text = '0'
    end
    object EdtMapY1: TEdit
      Left = 83
      Top = 72
      Width = 43
      Height = 21
      Alignment = taRightJustify
      Enabled = False
      ReadOnly = True
      TabOrder = 2
      Text = '0'
    end
    object EdtX1: TEdit
      Left = 20
      Top = 112
      Width = 33
      Height = 21
      Alignment = taRightJustify
      TabOrder = 3
      Text = '0'
      OnExit = EdtX1Exit
    end
    object EdtY1: TEdit
      Left = 83
      Top = 112
      Width = 33
      Height = 21
      Alignment = taRightJustify
      TabOrder = 4
      Text = '0'
      OnExit = EdtY1Exit
    end
    object CBHae1: TCheckBox
      Left = 146
      Top = 105
      Width = 137
      Height = 35
      Caption = 'Hae klikkaamalla karttaa (oikea n'#228'pp'#228'in)'
      TabOrder = 5
      WordWrap = True
      OnClick = CBHae1Click
    end
    object UpDownX1: TUpDown
      Left = 53
      Top = 110
      Width = 17
      Height = 25
      TabOrder = 6
      OnClick = UpDownX1Click
    end
    object UpDownY1: TUpDown
      Left = 116
      Top = 110
      Width = 17
      Height = 25
      TabOrder = 7
      OnClick = UpDownY1Click
    end
  end
  object GroupBox2: TGroupBox
    Left = 16
    Top = 175
    Width = 292
    Height = 145
    Caption = ' Toinen kohdistuspiste '
    TabOrder = 1
    object Label4: TLabel
      Left = 20
      Top = 16
      Width = 49
      Height = 13
      Caption = 'Rastikoodi'
    end
    object Label5: TLabel
      Left = 20
      Top = 56
      Width = 96
      Height = 13
      Caption = 'Sijainti kartalla (mm)'
    end
    object Label6: TLabel
      Left = 20
      Top = 96
      Width = 105
      Height = 13
      Caption = 'Sijainti n'#228'yt'#246'll'#228' (pixel)'
    end
    object Label10: TLabel
      Left = 131
      Top = 32
      Width = 154
      Height = 39
      Caption = 
        'Toinen kohdistuspiste vaikuttaa vain skaalaukseen, joka perustuu' +
        ' x-koordinaattiin '
      WordWrap = True
    end
    object EdtKdi2: TEdit
      Left = 20
      Top = 32
      Width = 33
      Height = 21
      Alignment = taRightJustify
      TabOrder = 0
      Text = '0'
      OnExit = EdtKdi2Exit
      OnKeyPress = EdtKdi2KeyPress
    end
    object EdtMapX2: TEdit
      Left = 20
      Top = 72
      Width = 44
      Height = 21
      Alignment = taRightJustify
      Enabled = False
      ReadOnly = True
      TabOrder = 1
      Text = '0'
    end
    object EdtMapY2: TEdit
      Left = 83
      Top = 72
      Width = 43
      Height = 21
      Alignment = taRightJustify
      Enabled = False
      ReadOnly = True
      TabOrder = 2
      Text = '0'
    end
    object EdtX2: TEdit
      Left = 20
      Top = 112
      Width = 33
      Height = 21
      Alignment = taRightJustify
      TabOrder = 3
      Text = '0'
      OnExit = EdtX2Exit
    end
    object EdtY2: TEdit
      Left = 83
      Top = 112
      Width = 33
      Height = 21
      TabStop = False
      Alignment = taRightJustify
      Enabled = False
      ReadOnly = True
      TabOrder = 4
      Text = '0'
      OnExit = EdtXYExit
    end
    object CBHae2: TCheckBox
      Left = 146
      Top = 110
      Width = 138
      Height = 26
      Caption = 'Hae klikkaamalla karttaa (oikea n'#228'pp'#228'in)'
      TabOrder = 5
      WordWrap = True
      OnClick = CBHae2Click
    end
    object UpDownX2: TUpDown
      Left = 53
      Top = 110
      Width = 17
      Height = 25
      TabOrder = 6
      OnClick = UpDownX2Click
    end
  end
  object GroupBox3: TGroupBox
    Left = 320
    Top = 24
    Width = 185
    Height = 169
    Caption = ' Karttakuvan lasketut kohdistukset '
    TabOrder = 2
    object Label8: TLabel
      Left = 20
      Top = 56
      Width = 98
      Height = 13
      Caption = 'Oikea alakulma (mm)'
    end
    object Label9: TLabel
      Left = 20
      Top = 96
      Width = 88
      Height = 13
      Caption = 'Kuvan koko (pixel)'
    end
    object Label7: TLabel
      Left = 20
      Top = 15
      Width = 100
      Height = 13
      Caption = 'Vasen yl'#228'kulma (mm)'
    end
    object Label11: TLabel
      Left = 16
      Top = 136
      Width = 147
      Height = 26
      Caption = 'T'#228'll'#228' alueella voi tehd'#228' lopuksi hienos'#228#228't'#246#228
      WordWrap = True
    end
    object EdtMapRight: TEdit
      Left = 20
      Top = 72
      Width = 49
      Height = 21
      Alignment = taRightJustify
      Enabled = False
      ReadOnly = True
      TabOrder = 0
      Text = '0'
    end
    object EdtMapBottom: TEdit
      Left = 89
      Top = 72
      Width = 46
      Height = 21
      Alignment = taRightJustify
      Enabled = False
      ReadOnly = True
      TabOrder = 1
      Text = '0'
    end
    object EdtMapWidth: TEdit
      Left = 21
      Top = 112
      Width = 48
      Height = 21
      Alignment = taRightJustify
      Enabled = False
      TabOrder = 2
      Text = '0'
    end
    object EdtMapHeight: TEdit
      Left = 89
      Top = 112
      Width = 46
      Height = 21
      Alignment = taRightJustify
      Enabled = False
      TabOrder = 3
      Text = '0'
    end
    object EdtMapLeft: TEdit
      Left = 20
      Top = 31
      Width = 49
      Height = 21
      Alignment = taRightJustify
      Enabled = False
      ReadOnly = True
      TabOrder = 4
      Text = '0'
    end
    object EdtMapTop: TEdit
      Left = 89
      Top = 31
      Width = 46
      Height = 21
      Alignment = taRightJustify
      Enabled = False
      ReadOnly = True
      TabOrder = 5
      Text = '0'
    end
    object UpDownMapL: TUpDown
      Left = 69
      Top = 29
      Width = 17
      Height = 25
      TabOrder = 6
      OnClick = UpDownMapLClick
    end
    object UpDownMapT: TUpDown
      Left = 136
      Top = 29
      Width = 17
      Height = 25
      TabOrder = 7
      OnClick = UpDownMapTClick
    end
    object UpDownMapR: TUpDown
      Left = 69
      Top = 70
      Width = 17
      Height = 25
      TabOrder = 8
      OnClick = UpDownMapRClick
    end
  end
  object Button1: TButton
    Left = 320
    Top = 215
    Width = 114
    Height = 49
    Caption = 'Karkea kohdistus (kaikki rastit kartalle)'
    TabOrder = 3
    WordWrap = True
    OnClick = Button1Click
  end
  object Button2: TButton
    Left = 320
    Top = 273
    Width = 114
    Height = 46
    Caption = 'Siirr'#228' kohdistus ratakaavakkeelle'
    TabOrder = 4
    WordWrap = True
    OnClick = Button2Click
  end
  object Button3: TButton
    Left = 456
    Top = 219
    Width = 67
    Height = 40
    Caption = 'Peruuta muutokset'
    TabOrder = 5
    WordWrap = True
    OnClick = Button3Click
  end
  object Button4: TButton
    Left = 456
    Top = 276
    Width = 67
    Height = 43
    Caption = 'Sulje kaavake'
    TabOrder = 6
    WordWrap = True
    OnClick = Button4Click
  end
  object BitBtn1: TBitBtn
    Left = 505
    Top = 10
    Width = 31
    Height = 31
    Margins.Left = 0
    Margins.Right = 0
    Margins.Bottom = 0
    Glyph.Data = {
      
      0000}
    TabOrder = 7
    TabStop = False
    WordWrap = True
    OnClick = BitBtn1Click
  end
end
