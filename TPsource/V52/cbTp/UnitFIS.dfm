object FormFIS: TFormFIS
  Left = 0
  Top = 0
  Caption = 'FIS-tiedostostojen lukeminen ja m'#228#228'rittely'
  ClientHeight = 489
  ClientWidth = 688
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object GroupBox1: TGroupBox
    Left = 32
    Top = 19
    Width = 158
    Height = 196
    Caption = ' FIS-pisteiden lukeminen '
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 0
    object Label1: TLabel
      Left = 50
      Top = 159
      Width = 54
      Height = 19
      Caption = 'Valmis'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      Visible = False
    end
    object RGLaji: TRadioGroup
      Left = 11
      Top = 20
      Width = 134
      Height = 61
      Caption = ' Laji '
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = []
      Items.Strings = (
        'Pitk'#228't matkat (DI)'
        'Sprint (SP)')
      ParentFont = False
      TabOrder = 0
    end
    object BtnLuePisteet: TButton
      Left = 22
      Top = 91
      Width = 116
      Height = 60
      Caption = 'Valitse tiedosto (CC???pts.csv) ja siirr'#228' pisteet'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 1
      WordWrap = True
      OnClick = BtnLuePisteetClick
    end
  end
  object GroupBox2: TGroupBox
    Left = 213
    Top = 19
    Width = 153
    Height = 196
    Caption = ' Nimien lukeminen '
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -12
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 1
    object Label2: TLabel
      Left = 47
      Top = 159
      Width = 54
      Height = 19
      Caption = 'Valmis'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -16
      Font.Name = 'Tahoma'
      Font.Style = [fsBold]
      ParentFont = False
      Visible = False
    end
    object BtnLueNimet: TButton
      Left = 20
      Top = 72
      Width = 113
      Height = 78
      Caption = 'Valitse tiedosto (CC???com.csv) ja siirr'#228' nimet ja maa-koodit'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clWindowText
      Font.Height = -12
      Font.Name = 'Tahoma'
      Font.Style = []
      ParentFont = False
      TabOrder = 0
      WordWrap = True
      OnClick = BtnLueNimetClick
    end
  end
  object BitBtn1: TBitBtn
    Left = 429
    Top = 17
    Width = 31
    Height = 31
    Margins.Left = 0
    Margins.Right = 0
    Margins.Bottom = 0
    Glyph.Data = {
      
      0000}
    TabOrder = 2
    TabStop = False
    OnClick = BitBtn1Click
  end
  object GroupBox3: TGroupBox
    Left = 8
    Top = 230
    Width = 461
    Height = 251
    Caption = ' Kilpailun tiedot FIS-tulosten laadintaan '
    TabOrder = 3
    object Label3: TLabel
      Left = 5
      Top = 15
      Width = 119
      Height = 13
      Caption = '(Sulkeissa esimerkkitieto)'
    end
    object Label4: TLabel
      Left = 256
      Top = 16
      Width = 114
      Height = 13
      Caption = 'Kilpailun muoto (F-arvo)'
    end
    object Label5: TLabel
      Left = 256
      Top = 53
      Width = 126
      Height = 13
      Caption = 'Kilpailun taso (min-pisteet)'
    end
    object Label6: TLabel
      Left = 256
      Top = 88
      Width = 127
      Height = 13
      Caption = 'Kilpailun luokka (Category)'
    end
    object BtnLue: TButton
      Left = 24
      Top = 214
      Width = 121
      Height = 25
      Caption = 'Lue aiempi tiedosto'
      TabOrder = 0
      OnClick = BtnLueClick
    end
    object BtnKirj: TButton
      Left = 169
      Top = 215
      Width = 137
      Height = 25
      Caption = 'Kirjoita m'#228#228'ritystiedosto'
      TabOrder = 16
      OnClick = BtnKirjClick
    end
    object EdtKoodi: TLabeledEdit
      Left = 108
      Top = 31
      Width = 56
      Height = 21
      EditLabel.Width = 101
      EditLabel.Height = 13
      EditLabel.Caption = 'Kilpailun koodi (3453)'
      LabelPosition = lpLeft
      TabOrder = 1
    end
    object EdtNimi: TLabeledEdit
      Left = 108
      Top = 53
      Width = 130
      Height = 21
      EditLabel.Width = 59
      EditLabel.Height = 13
      EditLabel.Caption = 'Kilpailun nimi'
      LabelPosition = lpLeft
      TabOrder = 2
    end
    object EdtPaikka: TLabeledEdit
      Left = 108
      Top = 98
      Width = 130
      Height = 21
      EditLabel.Width = 96
      EditLabel.Height = 13
      EditLabel.Caption = 'Kilpailupaikka (Lahti)'
      LabelPosition = lpLeft
      TabOrder = 4
    end
    object EdtPv: TLabeledEdit
      Left = 108
      Top = 142
      Width = 79
      Height = 21
      EditLabel.Width = 93
      EditLabel.Height = 13
      EditLabel.Caption = 'P'#228'iv'#228' (17.02.2014)'
      LabelPosition = lpLeft
      TabOrder = 6
    end
    object EdtKausi: TLabeledEdit
      Left = 108
      Top = 164
      Width = 56
      Height = 21
      EditLabel.Width = 60
      EditLabel.Height = 13
      EditLabel.Caption = 'Kausi (2014)'
      LabelPosition = lpLeft
      TabOrder = 7
    end
    object EdtLista: TLabeledEdit
      Left = 108
      Top = 186
      Width = 56
      Height = 21
      EditLabel.Width = 77
      EditLabel.Height = 13
      EditLabel.Caption = 'FIS-listan no (5)'
      LabelPosition = lpLeft
      TabOrder = 8
    end
    object EdtMaa: TLabeledEdit
      Left = 108
      Top = 120
      Width = 56
      Height = 21
      EditLabel.Width = 48
      EditLabel.Height = 13
      EditLabel.Caption = 'Maa (FIN)'
      LabelPosition = lpLeft
      TabOrder = 5
      Text = 'FIN'
    end
    object CBFvalue: TComboBox
      Left = 256
      Top = 31
      Width = 185
      Height = 21
      ItemIndex = 0
      TabOrder = 9
      Text = 'V'#228'liaikal'#228'ht'#246' (800)'
      Items.Strings = (
        'V'#228'liaikal'#228'ht'#246' (800)'
        'Sprint ja takaa-ajo (1200)'
        'Yhteisl'#228'ht'#246' ja skiathlon (1400)'
        'Sprintin kokonaisj'#228'rjestys (0)')
    end
    object CBminpist: TComboBox
      Left = 256
      Top = 68
      Width = 185
      Height = 21
      ItemIndex = 0
      TabOrder = 10
      Text = 'FIS-kilpailu (15)'
      Items.Strings = (
        'FIS-kilpailu (15)'
        'FIS Marathon Cup (25)'
        'FIS juniorikilpailu (35)'
        'EYOF (35)'
        'Youth Olympic Games(35)'
        'World Cup tai vast (aina 0)'
        'Maksimi 25'
        'Maksimi 35')
    end
    object EdtTDid: TLabeledEdit
      Left = 344
      Top = 126
      Width = 56
      Height = 21
      EditLabel.Width = 123
      EditLabel.Height = 13
      EditLabel.Caption = 'TD:n id-koodi (ei tarpeen)'
      LabelPosition = lpLeft
      TabOrder = 12
      Text = '0'
    end
    object EdtTDsnimi: TLabeledEdit
      Left = 344
      Top = 148
      Width = 93
      Height = 21
      EditLabel.Width = 66
      EditLabel.Height = 13
      EditLabel.Caption = 'TD:n sukunimi'
      LabelPosition = lpLeft
      TabOrder = 13
    end
    object EdtTDenimi: TLabeledEdit
      Left = 344
      Top = 170
      Width = 93
      Height = 21
      EditLabel.Width = 60
      EditLabel.Height = 13
      EditLabel.Caption = 'TD:n etunimi'
      LabelPosition = lpLeft
      TabOrder = 14
    end
    object EdtTDmaa: TLabeledEdit
      Left = 345
      Top = 192
      Width = 56
      Height = 21
      EditLabel.Width = 74
      EditLabel.Height = 13
      EditLabel.Caption = 'TD:n maa (FIN)'
      LabelPosition = lpLeft
      TabOrder = 15
    end
    object CBcategory: TComboBox
      Left = 256
      Top = 103
      Width = 185
      Height = 21
      ItemIndex = 0
      TabOrder = 11
      Text = 'FIS-kilpailu (FIS)'
      Items.Strings = (
        'FIS-kilpailu (FIS)'
        'FIS nuorten kilpailu (JUN)'
        'Kansall. mest.-kilp (NC)'
        'Nuorten kans.mest (NJC)'
        'Scandinavian Cup (SCAN)'
        'CISM'
        'Massakilpailut (ML)'
        '')
    end
    object EdtLaji: TLabeledEdit
      Left = 108
      Top = 75
      Width = 130
      Height = 21
      EditLabel.Width = 88
      EditLabel.Height = 13
      EditLabel.Caption = 'Kilp.laji (tyyli, mtk)'
      LabelPosition = lpLeft
      TabOrder = 3
    end
  end
  object BtnSulje: TButton
    Left = 385
    Top = 154
    Width = 75
    Height = 61
    Caption = 'Sulje   kaavake'
    TabOrder = 4
    WordWrap = True
    OnClick = BtnSuljeClick
  end
  object Memo1: TMemo
    Left = 475
    Top = 188
    Width = 205
    Height = 293
    Lines.Strings = (
      '')
    TabOrder = 5
  end
  object Button1: TButton
    Left = 475
    Top = 157
    Width = 205
    Height = 25
    Caption = 'Tallenna alla olevat m'#228#228'ritykset'
    TabOrder = 6
    OnClick = Button1Click
  end
  object OpenDialog1: TOpenDialog
    Left = 544
    Top = 80
  end
  object SaveDialog1: TSaveDialog
    Left = 504
    Top = 72
  end
end
