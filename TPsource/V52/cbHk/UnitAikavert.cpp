...
ColTp VertGridCols0[MAXAVCOL] = {
	...
	{L"Nimi", 90, 0, 1, 0, true},
	{L"Seura", 60, 0, 1, 0, false},
	...
	for (int ic = 0; ic < MAXAVCOL; ic++) {
		if (!VertGridCols[grid][ic].Visible)
		 ...
		 switch (ic) {
			...
			case 4:
				if (kilp.kilpno)
					Cells[grid][c][r].text = kilp.nimi(as, 35, false);
				else {
			...
			case 5:
				if (kilp.kilpno)
					Cells[grid][c][r].text = kilp.seuratieto(0);
				else
					Cells[grid][c][r].text = L"";
				break;
			case 6:
				...
				if (kilp.kilpno) {
					if (piste > -2 && (tl = kilp.p_aika(piste)) != 0) {
						if (piste == -1) {
							...
							Cells[grid][c][r].text = eroaikatowstr(as, tl, pvparam[k_pv].laika);
							}
						else {
							Cells[grid][c][r].text = aikatowstr_cols_n(as, tl, 0, 0, pvparam[k_pv].laika);
							}
						}
					else
						Cells[grid][c][r].text = L"";
			...
			case 8:
				if (kilp.kilpno) {
					if (piste > -2 && (tl = kilp.p_aika(piste)) != 0) {
						...
						if (ero1)
							Cells[grid][c][r].text = eroaikatowstr(as, ero, pvparam[k_pv].laika);
						else
							Cells[grid][c][r].text = L"";
						}
					}
			...
			case 9:
				if (kilp.kilpno)
					Cells[grid][c][r].text = (kilp.tark(k_pv) != L'-') ? UnicodeString(kilp.tark(k_pv)) : UnicodeString(L"");
				else
					Cells[grid][c][r].text = L"";
				break;
			case 10:
...
void __fastcall TFormAikavert::LuoUusi(int grid)
{
	...
	if (vpt.t/AIKAJAK != TMAALI0) {
		...
		lisaa_aika(Jono[grid], t, -2, vpt.t, vpt.date);
		}
	naytaAjat();
}
//---------------------------------------------------------------------------
...
void __fastcall TFormAikavert::FormCreate(TObject *Sender)
{
	...
	if (maxjono) {
		...
		CBJono1->Items->Add(L"Aika");
		CBJono1->Items->Add(L"Lähtö");
		CBJono2->Items->Add(L"Aika");
		CBJono2->Items->Add(L"Lähtö");
		}
	else {
	...
	Nytnimi1->Checked = VertGridCols[0][4].Visible;
	Nytseura1->Checked = VertGridCols[0][5].Visible;
	...
	Nytnimi2->Checked = VertGridCols[1][4].Visible;
	Nytseura2->Checked = VertGridCols[1][5].Visible;
...
void __fastcall TFormAikavert::BtnHakuClick(TObject *Sender)
{
	...
	if (aikajono[Jono[0]]->aktrows == 0 || kno < minkilpno || kno > maxkilpno)
		return;
	alkuRivi = aktRivi1;
...
void __fastcall TFormAikavert::Nytnimi1Click(TObject *Sender)
{
	...
	VertGridCols[0][4].Visible = Nytnimi1->Checked;
	SarakeLeveydet();
}
//---------------------------------------------------------------------------

void __fastcall TFormAikavert::Nytseura1Click(TObject *Sender)
{
	Nytseura1->Checked = !Nytseura1->Checked;
	VertGridCols[0][5].Visible = Nytseura1->Checked;
	SarakeLeveydet();
}
//---------------------------------------------------------------------------
...
void __fastcall TFormAikavert::Nytnimi2Click(TObject *Sender)
{
	...
	VertGridCols[1][4].Visible = Nytnimi2->Checked;
	SarakeLeveydet();
}
//---------------------------------------------------------------------------

void __fastcall TFormAikavert::Nytseura2Click(TObject *Sender)
{
	Nytseura2->Checked = !Nytseura2->Checked;
	VertGridCols[1][5].Visible = Nytseura2->Checked;
	SarakeLeveydet();
}
//---------------------------------------------------------------------------
...
void __fastcall TFormAikavert::Aktivoi(int grid, int rivi, int status)
{
	...
	upt = pt;
	upt.status = status;
	tall_rivi(Jono[grid], &upt, &pt, 0, 0, 0, 0, 0);
	naytaAjat();
}
//---------------------------------------------------------------------------
...
void __fastcall TFormAikavert::EdtAlkuChange(TObject *Sender)
{
	...
	naytaAjat();
	if (Alku1 <= ScrollBar1->Max && Alku1 >= ScrollBar1->Min)
