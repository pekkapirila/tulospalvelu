...
#pragma resource "*.dfm"
extern TFormAvausMaar *FormAvausMaar;
...
{
	TextFl *InitFile = new TextFl((FormMain->InitDir+UnicodeString(L"\\Init.cfg")).c_str(), L"w");
	if (InitFile->IsOpen()) {
		wchar_t line[200];
...
void __fastcall TFormAvausMaar::FormShow(TObject *Sender)
{
	...
	FormMain->lueInit(&TTila, &KsMoodi, &KLuett, &avaa, &cfgflnm);
	if (FormMain->CfgFileName.Length() > 0)
	   cfgflnm = FormMain->CfgFileName;
	Edit1->Text = cfgflnm;
