...
#pragma resource "*.dfm"
TFormTaulu *FormTaulu;

static int lahriviCPower(int rno, int rivi, char *teksti);
extern TauluParamTp TauluParam[NREGNLY];

extern TFormTaulu *FrmGAZ[2];
...
//---------------------------------------------------------------------------
__fastcall TFormTaulu::TFormTaulu(TComponent* Owner)
	: TForm(Owner)
{
	Scaled = false;
...
void TFormTaulu::PaivitaYhteydet(void)
{
	int y = 0;
	bool onLaite = false;
	...
	for (int ny = 0; ny < NREGNLY; ny++) {
		if (regnly[ny] == LID_CPOWER || regnly[ny] == LID_RTNM) {
			wchar_t st[30];
			...
			switch (comtype[cn_regnly[ny]]) {
				...
				case comtpUDPSTREAM :
					...
					SGTaulut->Cells[3][y+1] = ipparam[cn_regnly[ny]].srvport;
					SGTaulut->Cells[4][y+1] = ipparam[cn_regnly[ny]].destaddr;
					SGTaulut->Cells[5][y+1] = ipparam[cn_regnly[ny]].destport;
					break;
				}
...
void TFormTaulu::opentaulu(void)
   {
   int os, er = 1;
...
int TFormTaulu::taulu_tied(char *text1[], int nrv)
   {
   int i;
   char st[100];
...
int TFormTaulu::taulu_lah(char *text1[], int nrv)
   {
   int i, n = 0;
...
void TFormTaulu::paiv_taulu(void)
   {
   int k, os;
   int d, nt, rno;
...
//				int os;
				...
				switch (CBCPTieto[taulu][rivi]->ItemIndex) {
					...
					case 1:
						...
						sprintf(st, "%d.", os);
						st[strlen(st)] = ' ';
						sprintf(st + 3, "%4d", ntulos[taulusrj][0]);
						break;
					case 2:
						tls = KSEK*T_TIME(biostime(0,0));
						tkarki[os] = TMAALI0;
						if (ntulos[taulusrj][0]) {
							d = jalku[taulusrj][0];
							if (d > 0) {
								kilp.GETREC(d);
//								tkarki[os] = kilp.Maali(os, 0);
								}
							}
...
void TFormTaulu::closetaulu(void)
   {
   if (htaulu >= 0)
	  _close(htaulu);
   else
...
void TFormTaulu::nayta_valinnat(void)
{
	nosuus = 0;
...
void __fastcall TFormTaulu::Button1Click(TObject *Sender)
{
	for	(int i = 0; i < TAULU_NRIVI; i++) {
...
void __fastcall TFormTaulu::FormShow(TObject *Sender)
{
	nayta_valinnat();
	PaivitaYhteydet();
	if (cn_gaz)
...
void __fastcall TFormTaulu::CheckBox11Click(TObject *Sender)
{
	hidasta = CheckBox11->Checked;
}
//---------------------------------------------------------------------------

void TFormTaulu::PaivLEDsivu(void)
{
	int it = 0, nt = 0, yt;
...
//		if (rno > 4)
	...
	for (int it = 0; it < LED_LKM; it++) {
		...
		RBTyhja[it]->Top = RBTyhja[0]->Top;
		RBOletus[it]->Top = RBOletus[0]->Top;
		if ((nt = _wtoi(EdRiveja[it]->Text.c_str())) > 0 && nt <= LED_NRIVI)
			ledNRivi[it] = nt;
		else
			ledNRivi[it] = 1;
		EdRiveja[it]->Text = ledNRivi[it];
...
void __fastcall TFormTaulu::TyhjTaulu(int taulu)
{
	lahriviCPower(CPrno[taulu], 0, "  ");
}
//---------------------------------------------------------------------------

void __fastcall TFormTaulu::TauluAkt(int taulu, bool on)
{
	CPavoin[taulu] = on ? 1 : 0;
}
//---------------------------------------------------------------------------

void __fastcall TFormTaulu::TauluJako(int taulu, int nikk)
{
	UCHAR buf[100];
	...
	*(UINT16 *)(buf + 13) = 2 + 8*nikk;
	...
	for (int i = 0; i < nikk; i++) {
		WX[i] = 0;
		WY[i] = (TauluParam[CPrno[taulu]].Korkeus / nikk) * i;
		WW[i] = TauluParam[CPrno[taulu]].Leveys;
		WH[i] = TauluParam[CPrno[taulu]].Korkeus / nikk;
		buf[19+8*i] = WX[i] / 256;
		buf[20+8*i] = WX[i] % 256;
		buf[21+8*i] = WY[i] / 256;
		buf[22+8*i] = WY[i] % 256;
...
void __fastcall TFormTaulu::TauluKirkkaus(int taulu, int val)
{
	UCHAR buf[60];
	UINT16 chksum = 0;
	int nsent;
...
void __fastcall TFormTaulu::Naytaoletus(int taulu)
{
	UCHAR buf[30];
	UINT16 chksum = 0;
	int nsent;
...
void __fastcall TFormTaulu::BtnAsKelloClick(TObject *Sender)
{
...
void __fastcall TFormTaulu::SGTaulutMouseDown(TObject *Sender, TMouseButton Button,
...
void __fastcall TFormTaulu::LaiteYhtClick(TObject *Sender)
{
...
void __fastcall TFormTaulu::LaiteClick(TObject *Sender)
{
	if (Sender == miGAZ)
		...
		SGTaulut->Cells[SGTaulut->Col][SGTaulut->Row] = L"D-RTNM";
		}
}
//---------------------------------------------------------------------------

void __fastcall TFormTaulu::BtnAvaaYhtClick(TObject *Sender)
{
	...
	if (wmemcmpU(SGTaulut->Cells[1][y+1].c_str(), L"GAZ", 3) == 0) {
		...
		swscanf(SGTaulut->Cells[7][y+1].c_str(), L"%d.%d.%d.%d", &i1, &i2, &i3, &i4);
	...
	else if (wmemcmpU(SGTaulut->Cells[1][y+1].c_str(), L"D-RTNM", 3) == 0) {
		UINT32 i1 = 0, i2 = 0, i3 = 0, i4 = 0;
		regnly[ny] = LID_RTNM;
		swscanf(SGTaulut->Cells[7][y+1].c_str(), L"%d.%d.%d.%d", &i1, &i2, &i3, &i4);
		TauluParam[ny].Leveys = SGTaulut->Cells[8][y+1].ToInt();
...
void __fastcall TFormTaulu::BtnSuljeYhtClick(TObject *Sender)
{
	int ny;
...
void __fastcall TFormTaulu::RBTilaClick(TObject *Sender)
{
	for (int taulu = 0; taulu < NLed; taulu++) {
		...
		if (Sender == RBOn[taulu]) {
			int n;
			if ((n = _wtoi(EdRiveja[taulu]->Text.c_str())) > 0 && n <= LED_NRIVI && RBOn[taulu]->Checked) {
				TauluJako(taulu, n);
				}
			TauluAkt(taulu, true);
			return;
...
void __fastcall TFormTaulu::TabSheet2Show(TObject *Sender)
{
	PaivLEDsivu();
}
//---------------------------------------------------------------------------

void __fastcall TFormTaulu::UpDownClick(TObject *Sender, TUDBtnType Button)
{
	for	(int it = 0; it < NLed; it++) {
		if (Sender == UpDown[it]) {
			EdKirk[it]->Text = UpDown[it]->Position;
			TauluKirkkaus(it, UpDown[it]->Position);
			break;
			}
		}
}
...
void __fastcall TFormTaulu::EdKirkExit(TObject *Sender)
{
	for	(int it = 0; it < NLed; it++) {
		if (Sender == EdKirk[it]) {
...
void __fastcall TFormTaulu::EdRivejaChange(TObject *Sender)
{
	for	(int it = 0; it < NLed; it++) {
		if (Sender == EdRiveja[it]) {
			int n;
			if ((n = _wtoi(EdRiveja[it]->Text.c_str())) > 0 && n <= LED_NRIVI && RBOn[it]->Checked) {
				TauluJako(it, n);
				if (n != ledNRivi[it])
					PaivLEDsivu();
				}
...
void __fastcall TFormTaulu::Button2Click(TObject *Sender)
{
	static bool init = false;
	char LEDoff[6] = "\x30\x01\x08\x01\x00\x00";
	char LEDon[6] = "\x30\x01\x08\x01\x00\x01";
	int ny, nsent, len, pos, alku;
	static int rr = 0;
	char buf[400], buf2[100] = "\x4a\x57", *p;

	for (ny = 0; ny < NREGNLY; ny++)
		if (regnly[ny] == LID_RTNM)
			break;
	if (ny == NREGNLY) {
		Application->MessageBoxW(L"Ei yhteyttä D-RTNM-tauluun", L"Este", MB_OK);
		return;
		}
	memset(buf, 0, sizeof(buf));
	for (int r = 0; r < 10; r++) {
		wcstoansi(buf+strlen(buf), Memo1->Lines->Strings[r].c_str(), 398);
		if (strlen(buf) > 0 && buf[strlen(buf+4)-1] != '\r') {
			strcat(buf, "\r");
		}
		if (strlen(buf) > 197) {
			break;
	...
	while (strlen(p) > 0) {
		...
		for (int i = 0; i < 10; i++) {
			...
			err = wrt_st_UDP_0(hComm[cn_regnly[ny]], len+4, buf2, &nsent);
			Sleep(30);
			if (!err) {
				for (int j = 0; i < 5; i++) {
					err = read_UDPcli_0(hComm[cn_regnly[ny]], 200, vast, &nread);
					if (!err && nread == 5 && !vast[4] && vast[3] == buf2[3]) {
						int k = 0;
						AnsiString as = "00", as1;
						for (int m = 0; m < nread; m++) {
							as1 = IntToHex(vast[m], 2);
							as = as + " " + as1;
							k++;
							if (k > 15 || m == nread-1) {
								char ss[6];
